...
const char * const CRYPTOTALKCOIN_CONF_FILENAME = "cryptotalkcoin.conf";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "cryptotalkcoind -foo=bar
...
#else
    const char* pszModule = "cryptotalkcoin";
#endif
    if (pex)
        return strprintf(
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Cryptotalkcoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Cryptotalkcoin
    // Mac: ~/Library/Application Support/Cryptotalkcoin
    // Unix: ~/.cryptotalkcoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Cryptotalkcoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Cryptotalkcoin";
#else
    // Unix
    return pathRet / ".cryptotalkcoin";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", CRYPTOTALKCOIN_CONF_FILENAME);
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Check for untranslated substitution to make sure Cryptotalkcoin Core copyright is not removed by accident
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Cryptotalkcoin Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The Cryptotalkcoin Core developers";
    }
