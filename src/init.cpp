 ...
 */
static const char* CRYPTOTALKCOIN_PID_FILENAME = "cryptotalkcoind.pid";

static fs::path GetPidFile()
{
    return AbsPathForConfigVal(fs::path(gArgs.GetArg("-pid", CRYPTOTALKCOIN_PID_FILENAME)));
}
 ...
 * between cryptotalkcoind, and cryptotalkcoin-qt and non-server tools.
*/
...
void SetupServerArgs()
{
    ...
    gArgs.AddArg("-conf=<file>", strprintf("Specify configuration file. Relative paths will be prefixed by datadir location. (default: %s)", CRYPTOTALKCOIN_CONF_FILENAME), false, OptionsCategory::OPTIONS);
    ...
    gArgs.AddArg("-pid=<file>", strprintf("Specify pid file. Relative paths will be prefixed by a net-specific datadir location. (default: %s)", CRYPTOTALKCOIN_PID_FILENAME), false, OptionsCategory::OPTIONS);
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/cryptotalkcoin/cryptotalkcoin>";
    const std::string URL_WEBSITE = "<https://cryptotalkcoincore.org>";
...
/** Sanity checks
 *  Ensure that Cryptotalkcoin is running in a usable environment with all
...
{
    // Make sure only a single Cryptotalkcoin process is using the data directory.
...
bool AppInitMain(InitInterfaces& interfaces)
{
    ...
    fs::path config_file_path = GetConfigFile(gArgs.GetArg("-conf", CRYPTOTALKCOIN_CONF_FILENAME));
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if cryptotalkcoin is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if cryptotalkcoin is started while in a temporary directory.\n",
