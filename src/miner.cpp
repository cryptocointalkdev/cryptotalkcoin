...
void static CryptotalkcoinMiner(const CChainParams& chainparams, std::shared_ptr<CReserveScript> coinbase_script)
{
	...
	try {
		...
		while (true) {
			...
			//LogPrintf("Running CryptotalkcoinMiner with %u transactions in block \n", pblock->vtx.size());
	...
	{
		LogPrintf("CryptotalkcoinMiner terminated\n");
		throw;
	}
	catch (const std::runtime_error &e)
	{
		LogPrintf("CryptotalkcoinMiner runtime error: %s\n", e.what());
		return;
	}
}

void GenerateCryptotalkcoins(bool fGenerate, int nThreads, const CChainParams& chainparams, std::shared_ptr<CReserveScript> coinbase_script)
{
	...
	for (int i = 0; i < nThreads; i++)
		minerThreads->create_thread(boost::bind(&CryptotalkcoinMiner, boost::cref(chainparams), boost::cref(coinbase_script)));
}
