...
###   cryptotalkcoincore.org and cryptotalkcoin.org and compares them.
...
WORKINGDIR="/tmp/cryptotalkcoin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://cryptotalkcoincore.org"
HOST2="https://cryptotalkcoin.org"
BASEDIR="/bin/"
VERSIONPREFIX="cryptotalkcoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'cryptotalkcoin-',
...
if [ $? -ne 0 ]; then
   echo "cryptotalkcoin.org failed to provide signature file, but cryptotalkcoincore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "cryptotalkcoin.org and cryptotalkcoincore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Cryptotalkcoin Core binary release signing key installed?"
   fi
