...
TEST_CRYPTOTALKCOIN_BINARY="src/test/test_cryptotalkcoin"
...
if [[ ! -e ${TEST_CRYPTOTALKCOIN_BINARY} ]]; then
    echo "Error: Executable ${TEST_CRYPTOTALKCOIN_BINARY} not found. Run \"./configure --enable-lcov\" and compile."
    exit 1
fi
...
while [[ ${TEST_RUN_ID} -lt ${N_TEST_RUNS} ]]; do
    ...
    if ! BOOST_TEST_RUN_FILTERS="${BOOST_TEST_RUN_FILTERS}" ${TEST_CRYPTOTALKCOIN_BINARY} > "${TEST_OUTPUT_TEMPFILE}" 2>&1; then
